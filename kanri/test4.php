    ...
    ?>
        ...
        ?>
            ...
            <form method=post>
                ...
                <!-- <input type="number" id="input_rows" name="Asa_rows" value="<?= $AsaData ?>" onchange="sendData(this.value,'Asa')"> -->
...
<script>
    ...
    function generateTableRow($timePeriod, $receivedData)
    {
        ...
        if ($timePeriod == "朝") {
            echo '
                ...
                <option value="'.(isset($_POST["朝_時間帯_始"]) ? date('H',strtotime($_POST["朝_時間帯_始"])) : '').'">'.(isset($_POST["朝_時間帯_始"]) ? date('H',strtotime($_POST["朝_時間帯_始"])) : '').'</option>
                </select>
                
                <span>:</span>
                <select id="AsastartMinutes" name="AsaStartminutes" onchange="syncInput(this.value, \'Full_StartMinute\')">
                    <option value="'.(isset($_POST["朝_時間帯_始"]) ? date('i',strtotime($_POST["朝_時間帯_始"])) : '').'">'.(isset($_POST["朝_時間帯_始"]) ? date('i',strtotime($_POST["朝_時間帯_始"])) : '').'</option>
                ...
                <select id="AsaendHours" name="AsaEndhours"  onchange="syncInput(this.value, \'HirustartHours\')">
                    <option value="'.(isset($_POST["朝_時間帯_終"]) ? date('H',strtotime($_POST["朝_時間帯_終"])) : '').'">'.(isset($_POST["朝_時間帯_終"]) ? date('H',strtotime($_POST["朝_時間帯_終"])) : '').'</option>
                </select>
            
                <span>:</span>
            
                <select id="AsaendMinutes" name="AsaEndminutes" onchange="syncInput(this.value, \'HirustartMinutes\')">
                    <option value="'.(isset($_POST["朝_時間帯_終"]) ? date('i',strtotime($_POST["朝_時間帯_終"])) : '').'">'.(isset($_POST["朝_時間帯_終"]) ? date('i',strtotime($_POST["朝_時間帯_終"])) : '').'</option> <!-- 他の分オプション -->
                </select>
            ...
            // echo '<input type="time" name="朝_時間帯_始" id="朝_時間帯_始" value="' . (isset($_POST["朝_時間帯_始"]) ? $_POST["朝_時間帯_始"] : '') . '" oninput="syncInput(this.value, \'総合計時間帯_始\')">';
            // echo ' ~ ';
            // echo '<input type="time" name="朝_時間帯_終" id="朝_時間帯_終" value="' . (isset($_POST["朝_時間帯_終"]) ? $_POST["朝_時間帯_終"] : '') . '" oninput="syncInput(this.value, \'昼_時間帯_始\')">';
        } else if ($timePeriod == "昼") {
            echo '
                <select id="HirustartHours" name="HiruStarthours" onchange="syncInput(this.value, \'AsaendHours\')">
                    <option value="'.(isset($_POST["昼_時間帯_始"]) ? date('H',strtotime($_POST["昼_時間帯_始"])):'').'">'.(isset($_POST["昼_時間帯_始"]) ? date('H',strtotime($_POST["昼_時間帯_始"])):'').'</option>
                </select>
                <span>:</span>
                <select id="HirustartMinutes" name="HiruStartminutes" onchange="syncInput(this.value, \'AsaendMinutes\')">
                    <option value="'.(isset($_POST["昼_時間帯_始"]) ? date('i',strtotime($_POST["昼_時間帯_始"])):'').'">'.(isset($_POST["昼_時間帯_始"]) ? date('i',strtotime($_POST["昼_時間帯_始"])):'').'</option>
                </select>
            
                <span>~</span>
            
                <select id="HiruendHours" name="HiruEndhours" onchange="syncInput(this.value, \'YorustartHours\')">
                    <option value="'.(isset($_POST["昼_時間帯_終"]) ? date('H',strtotime($_POST["昼_時間帯_終"])):'').'">'.(isset($_POST["昼_時間帯_終"]) ? date('H',strtotime($_POST["昼_時間帯_終"])):'').'</option>
                </select>
                <span>:</span>
                <select id="HiruendMinutes" name="HiruEndminutes" onchange="syncInput(this.value, \'YorustartMinutes\')">
                    <option value="'.(isset($_POST["昼_時間帯_終"]) ? date('i',strtotime($_POST["昼_時間帯_終"])):'').'">'.(isset($_POST["昼_時間帯_終"]) ? date('i',strtotime($_POST["昼_時間帯_終"])):'').'</option> <!-- 他の分オプション -->
                </select>
            ...
            // echo '<input type="time" name="昼_時間帯_始" id="昼_時間帯_始" value="' . (isset($_POST["昼_時間帯_始"]) ? $_POST["昼_時間帯_始"] : '') . '"  readonly>';
            // echo ' ~ ';
            // echo '<input type="time" name="昼_時間帯_終" id="昼_時間帯_終" value="' . (isset($_POST["昼_時間帯_終"]) ? $_POST["昼_時間帯_終"] : '') . '" oninput="syncInput(this.value, \'夜_時間帯_始\')">';
        } else {
            echo '
            <select id="YorustartHours" name="YoruStarthours" onchange="syncInput(this.value, \'HiruendHours\')">
            <option value="'.(isset($_POST["夜_時間帯_始"]) ? date('H',strtotime($_POST["夜_時間帯_始"])) : '').'">'.(isset($_POST["夜_時間帯_始"]) ? date('H',strtotime($_POST["夜_時間帯_始"])) : '').'</option>
      
          </select>
          <span>:</span>
          <select id="YorustartMinutes" name="YoruStartminutes" onchange="syncInput(this.value, \'HiruendMinutes\')">
            <option value="'.(isset($_POST["夜_時間帯_始"]) ? date('i',strtotime($_POST["夜_時間帯_始"])) : '').'">'.(isset($_POST["夜_時間帯_始"]) ? date('i',strtotime($_POST["夜_時間帯_始"])) : '').'</option>
          ...
          <select id="YoruendHours" name="YoruEndhours" onchange="syncInput(this.value, \'Full_EndHour\')">
            <option value="'.(isset($_POST["夜_時間帯_終"]) ? date('H',strtotime($_POST["夜_時間帯_終"])) : '').'">'.(isset($_POST["夜_時間帯_終"]) ? date('H',strtotime($_POST["夜_時間帯_終"])) : '').'</option>
      
          </select>
          <span>:</span>
          <select id="YoruendMinutes" name="YoruEndminutes" onchange="syncInput(this.value, \'Full_EndMinute\')">
            <option value="'.(isset($_POST["夜_時間帯_終"]) ? date('i',strtotime($_POST["夜_時間帯_終"])) : '').'">'.(isset($_POST["夜_時間帯_終"]) ? date('i',strtotime($_POST["夜_時間帯_終"])) : '').'</option> <!-- 他の分オプション -->
          </select>
          ...
          <input id="YoruEndtimeRange" type="hidden" name="夜_時間帯_終">
            ...
            // echo '<input type="time" name="夜_時間帯_始" id="夜_時間帯_始" value="'.(isset($_POST["夜_時間帯_始"]) ? $_POST["夜_時間帯_始"] : '').'" readonly>';
            // echo ' ~ ';
            // echo '<input type="time" name="夜_時間帯_終" id="夜_時間帯_終" value="' . (isset($_POST["夜_時間帯_終"]) ? $_POST["夜_時間帯_終"] : '') . '" oninput="syncInput(this.value, \'総合計時間帯_終\')">';
        }
        ...
        for ($j = 1; $j <= $receivedData; $j++) {
            echo '<td>';
            echo '<input type="text" name="' . $timePeriod . '_ポジション_' . $j . '" value="' . (isset($_POST[$timePeriod . "_ポジション_" . $j]) ? $_POST[$timePeriod . "_ポジション_" . $j] : '') . '">';
            echo '</td>';
            echo '<td>';
            // echo '<input type="number" name="' . $timePeriod . '_必要人数_' . $j . '" value="' . (isset($_POST[$timePeriod . "_必要人数_" . $j]) ? $_POST[$timePeriod . "_必要人数_" . $j] : '') . '">';
            echo '<input type="number" name="' . $timePeriod . '_必要人数_' . $j . '" id="' . $timePeriod . '_必要人数_' . $j . '" value="' . (isset($_POST[$timePeriod . "_必要人数_" . $j]) ? $_POST[$timePeriod . "_必要人数_" . $j] : 0) . '" oninput="preInput( \'' . $timePeriod . '\', \'' . $timePeriod . '_合計\', ' . $receivedData . ',' . $j . ')">';
            echo '</td>';
            echo '</tr>';
            echo '<tr>';
        }
        ...
        echo '<td>合計</td>';
        echo '<td>';
        echo '<input type="number" name="' . $timePeriod . '_合計" id="' . $timePeriod . '_合計" value="' . (isset($_POST[$timePeriod . "_合計"]) ? $_POST[$timePeriod . "_合計"] : 0) . '" readonly>';
        // echo '<input type="number" name="' . $timePeriod . '_合計"  id="' . $timePeriod . '_合計" value="0">';
        echo '</td>';
        echo '</tr>';
    }
...
//'.date('i',strtotime($data['昼']['時間帯']['始'])).'
      ...
      <input id="YoruEndtimeRange" type="hidden" name="夜_時間帯_終">
        ...
        for($j = 1; $j < $receivedData; $j++){
            ...
            if( $j==$receivedData){
                echo '<button id="'.$timePeriod.'_'.$j.'_RowPlus" value="'.$timePeriod.','.$j.'" onclick="RowPlus(this.value)">+</button>';
            }
        }
